...
**FastAPI-додаток для онлайн-кінотеатру.**  
Деплой виконано на AWS EC2, доступний за адресою:

👉 **[http://63.180.135.237/docs](http://63.180.135.237/docs)**
...
|-------------|--------------|
| **FastAPI** | Бекенд та OpenAPI-документація |
| **SQLite** | База даних |
| **SQLAlchemy (async)** | ORM |
| **Celery + Redis** | Фонові задачі |
| **Mailhog** | Тестування відправки email |
| **Docker & Docker Compose** | Контейнеризація |
| **Nginx** | Реверс-проксі для доступу ззовні |
...
```
⚠️Цей .env використовується як для локального запуску, так і для продакшену в Docker.
...
API розгорнуто у **Docker-контейнерах** з **Nginx-проксі**.

| Порт | Призначення |
|------|--------------|
| **80** | HTTP-доступ до FastAPI через Nginx |
| **8025** | Mailhog (перевірка email-повідомлень) |

📍 **Swagger UI (публічний доступ)**:  
👉 [http://63.180.135.237/docs](http://63.180.135.237/docs)

---
## 🔒 Авторизація

Для тестування API використовуй готові облікові записи:
...
> 🔑 Використай запит **POST** `/api/v1/accounts/login/`,  
> щоб отримати **JWT-токен** для авторизованих запитів.

...
class TestResetPasswordRequest(TestAccount):
    async def test_reset_password_request_success(self, test_app, monkeypatch):
        monkeypatch.setattr(
            "app.core.notifications.emails.EmailSender.send_password_reset_email",
            TestAccount.mock_send_email
        )
        hashed = hash_password("Password123!")
        async with engine.begin() as conn:
        ...
        transport = ASGITransport(app=test_app)
        async with AsyncClient(transport=transport, base_url="http://test") as ac:
            response = await ac.post("/api/v1/accounts/reset-password/request/", json={
                "email": "exists1@example.com",
            })
...
class TestResetPasswordComplete(TestAccount):
    async def test_reset_password_complete_success(self, test_app, monkeypatch):
        monkeypatch.setattr(
            "app.core.notifications.emails.EmailSender.send_password_reset_complete_email",
            TestAccount.mock_send_email
        )
        async with engine.begin() as conn:
            result = await conn.execute(
        ...
        transport = ASGITransport(app=test_app)
        async with AsyncClient(transport=transport, base_url="http://test") as ac:
            response = await ac.post("/api/v1/accounts/reset-password/complete/", json={
                "token": token_value,
                "email": self.email,
...
class TestTokenRefresh(TestAccount):
    async def test_token_refresh_success(self, test_app):
        ...
        transport = ASGITransport(app=test_app)
        async with AsyncClient(transport=transport, base_url="http://test") as ac:
            response = await ac.post("/api/v1/accounts/token/refresh/", json={
                "refresh_token": jwt_refresh_token,
            })
...
class TestAdminChangeGroup(TestAccount):
    async def test_admin_change_user_group_success(self, test_app):
        async with engine.begin() as conn:
            ...
            await conn.execute(insert(UserGroupModel).values(name=UserGroupEnum.ADMIN))
            await conn.execute(
                insert(UserModel).values(
                    email=self.email,
                    hashed_password=hash_password("Password123!"),
        ...
        jwt_access_token = jwt_manager.create_access_token({"user_id": admin_id})
        headers = {"Authorization": f"Bearer {jwt_access_token}"}

        transport = ASGITransport(app=test_app)
        async with AsyncClient(transport=transport, base_url="http://test") as ac:
            response = await ac.post("/api/v1/accounts/admin/change-user-group/", json={
                "email": self.email,
                "new_group": "moderator"
            },
                                     headers=headers
...
class TestActivateUser(TestAccount):
    async def test_admin_activate_user_success(self, test_app):
        ...
        transport = ASGITransport(app=test_app)
        async with AsyncClient(transport=transport, base_url="http://test") as ac:
            response = await ac.post("/api/v1/accounts/admin/activate-user/", json={
                "email": self.email,
            },
                                     headers=headers
                                     )
    ...
    async def test_admin_activate_user_already_activate(self, test_app):
        ...
        assert response.status_code == 400
        assert response.json() == {"detail": "User not found or user already active."}
...
class TestResendActivation(TestAccount):
    async def test_resend_activation_success(self, test_app, monkeypatch):
        ...
        transport = ASGITransport(app=test_app)
        async with AsyncClient(transport=transport, base_url="http://test") as ac:
            response = await ac.post("/api/v1/accounts/resend-activation/", json={
                "email": self.email,
            })
    ...
    async def test_resend_activation_already_activate(self, test_app):
        ...
        assert response.status_code == 400
        assert response.json() == {"detail": "Activation link already sent. Please try again later."}
